...
export class InPatientComponent implements OnInit {
  ...
  getAllAdmission() {
    this.admissionService.getAdmissionApi().subscribe((res) => {
      this.admissionList$ = res.filter(
        (opdOnly: { admissionTypeId: number }) => opdOnly.admissionTypeId == 2
      );
  ...
  modalAdd() {
    ...
    this.modalTitle = 'Add OPD Patient';
  ...
  exportToExcel(): void {
    ...
    let element = document.getElementById('in-patient-sch-table');
