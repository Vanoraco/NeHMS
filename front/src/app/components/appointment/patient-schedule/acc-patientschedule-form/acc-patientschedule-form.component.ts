...
@Component({
  selector: 'app-acc-patientschedule-form',
  templateUrl: './acc-patientschedule-form.component.html',
  styleUrls: ['./acc-patientschedule-form.component.css'],
})
export class AccPatientscheduleFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.employeeId$ = this.empService.getEmployeeApi();
  ...
  getDoctorOnly() {
    this.empService.getEmployeeApi().subscribe((res) => {
      ...
      for (let index = 0; index < this.employeeId$.length; index++) {
        const roleId = this.employeeId$[index].employeeRoleId;
        if (roleId) {
  ...
  getDoctorSchedule() {
    this.patientScheduleService.getScheduleApi().subscribe((res) => {
      this.doctorSchedule = res.filter(
        (employee: { employeeId: number }) => employee.employeeId == 17
      );
