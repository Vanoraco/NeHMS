...
@Component({
  selector: 'app-paid-patient-schedule',
  templateUrl: './paid-patient-schedule.component.html',
  styleUrls: ['./paid-patient-schedule.component.css'],
})
export class PaidPatientScheduleComponent implements OnInit {
  ...
  fileName = 'paid-patient-sch-report.xlsx';
      ...
      this.patientScheduleList$ = response
        .filter(
          (patientScheduleData: { is_Payed: boolean }) =>
            patientScheduleData.is_Payed == true
        )
        .reverse();
    });
  }
  ...
  exportToExcel(): void {
    ...
    let element = document.getElementById('paid-patient-sch-table');
